...
interface ChatStore {
  ...
  fetchUserCnt: (token?: string) => Promise<void>;
  clearSessions: () => void;
  ...
  selectSiderTitle: (name: string, useCnt: string) => void;
...
export const useChatStore = create<ChatStore>()(
  persist(
    (set, get) => ({
      ...
      selectSiderTitle(name: string, useCnt: string) {
        set({
          sidebarTitle: `欢迎 ${name}`,
          sidebarSubTitle: useCnt
        });
      },      
      ...
      async fetchUserCnt(token) {
        if (!token) {
          get().selectSiderTitle('欢迎使用', "请填写 设置->研发云Token");
          return;
        }
        ...
        fetch("https://dev.iwhalecloud.com/faas/serverless/gpt-api-gw/subscription", requestOptions)
          .then(response => response.json())
          .then((res) => {
            /**
             * {
                    "msg": "每天限定使用量200次(您还剩余198次)",
                    "name": "王荣"
                }
             */
            get().selectSiderTitle(res.name, res.msg);
          })
